...
class Transform(torch.nn.Module):
    def __init__(self, b, kpdetector, scale=0.35, angle=30, shear=30, shift=0.3):
        ...
        h = torch.tan((angle/180)*3.14159265359*(torch.rand((b, 1)) - 0.5)).unsqueeze(-1) #(b, 1, 1)
        v = torch.tan((angle/180)*3.14159265359*(torch.rand((b, 1)) - 0.5)).unsqueeze(-1) #(b, 1, 1)
        one = torch.ones((b, 1)).unsqueeze(-1) #(b, 1, 1)
