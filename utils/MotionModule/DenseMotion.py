...
class DenseMotion(torch.nn.Module):
    ...
    def forward(self, frame_source, kp_source, kp_driving):
        ...
        H = heatmap_diff(kp_source, kp_driving, (h, w)) #(b, num_kp + 1, h, w)
        ...
        map = self.bottle_neck(torch.cat([H.unsqueeze(2), source_df], dim=2).view(b, (self.num_kp + 1)*(c + 1), h, w)) #(b, layer_xp + (num_kp + 1)*(c + 1), h, w)
